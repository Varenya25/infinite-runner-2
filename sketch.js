...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if (ground.x < camera.position.x-300){
      ground.x = camera.position.x + 700
    }
    ...
    gameOver = createSprite(camera.position.x,100,10,10);
    gameOver.addImage(gameOverImg);
    gameoverGRP.add(gameOver);
    
  
    restart = createSprite(camera.position.x,140,10,10);
    restart.addImage(restartImg);
    gameoverGRP.add(restart);
    gameOver.scale = 0.5;
    restart.scale = 0.5;
